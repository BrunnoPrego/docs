...
O pacote de validação do CakePHP provê características para construir validadores
que podem validar vetores arbitrários com facilidade. Você pode encontrar uma `lista
validadores validos na API
...
Objetos Validator definem regras que se aplicam a um conjunto de campos.
Objetos Validator contém um mapeamento entre campos e conjuntos de validação. 
E os conjuntos de validação contém uma coleção de regras que se aplicam aos campos
que ele está anexado. Criar um validador é simples::

    use Cake\Validation\Validator;

    $validator = new Validator();

Uma vez criado, você pode definir conjuntos de regras que deseja validar::

    $validator
        ->requirePresence('title')
        ->notEmpty('title', 'Gentileza informar o titulo')
        ->add('title', [
            'length' => [
                'rule' => ['minLength', 10],
                'message' => 'Os titulos devem ter mais de 10 caracteres',
            ]
        ])
        ...
        ->add('body', 'length', [
            'rule' => ['minLength', 50],
            'message' => 'Artigos devem ter conteúdo considerável.'
        ]);

Como exemplificado acima, os validadores são construídos com uma interface fluente que
permite definir regras para cada campo que você deseja validar.

Alguns métodos foram chamados no exemplo acima, então vamos avançar nas várias 
características. O método ``add()`` permite incluir novas regras a um validador. 
Você pode tanto incluir regras individualmente ou em grupos como mostrado acima.

Definindo campos obrigatórios
-----------------------------

O método ``requirePresence()`` exige que o campo esteja presente em qualquer vetor validado. 
Se o campo estiver ausente, a validação vai falhar. O método ``requirePresence()`` tem 4 modos:

* ``true`` O campo é sempre exigido.
* ``false`` O campo não é exigido.
* ``create`` A presença do campo é exigida quando valida uma operação de **create**.
* ``update`` A presença do campo é exigida quando valida uma operação de **update**.

Por padrão, ``true`` é usado. A presença da chave é verificada com ``array_key_exists()`` 
então valores null contam como presentes. Você pode definir o modo usando um segundo parametro::

    $validator->requirePresence('author_id', 'create');

Se você tem multiplos campos que são requeridos, você pode definir eles com uma lista::

    // Define multiplos campos como create
    $validator->requirePresence(['author_id', 'title'], 'create');

    // Define multiplos campos com modos variados
    $validator->requirePresence([
        'author_id' => [
            'mode' => 'create',
            'message' => 'Um autor é obrigatório.',
        ],
        'published' => [
            'mode' => 'update',
            'message' => 'Estado de publicação é obrigatório.',
        ]
    ]);

.. versionadded:: 3.3.0
    ``requirePresence()`` aceita vetores de campos desde a versão 3.3.0

Permitindo campos vazios
------------------------
